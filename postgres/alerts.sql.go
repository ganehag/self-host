...
func (q *Queries) DeleteAlert(ctx context.Context, argUuid uuid.UUID) (int64, error) {
	result, err := q.exec(ctx, q.deleteAlertStmt, deleteAlert, argUuid)
	if err != nil {
		return 0, err
	}
...
func (q *Queries) ExistsAlert(ctx context.Context, argUuid uuid.UUID) (int64, error) {
	row := q.queryRow(ctx, q.existsAlertStmt, existsAlert, argUuid)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) FindAlertByUUID(ctx context.Context, argUuid uuid.UUID) (VAlert, error) {
	row := q.queryRow(ctx, q.findAlertByUUIDStmt, findAlertByUUID, argUuid)
	var i VAlert
	err := row.Scan(
		&i.Uuid,
		&i.Resource,
...
func (q *Queries) UpdateAlertIncDuplicate(ctx context.Context, argUuid uuid.UUID) (int64, error) {
	result, err := q.exec(ctx, q.updateAlertIncDuplicateStmt, updateAlertIncDuplicate, argUuid)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
...
func (q *Queries) UpdateAlertSetLastReceivedTime(ctx context.Context, argUuid uuid.UUID) (int64, error) {
	result, err := q.exec(ctx, q.updateAlertSetLastReceivedTimeStmt, updateAlertSetLastReceivedTime, argUuid)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
